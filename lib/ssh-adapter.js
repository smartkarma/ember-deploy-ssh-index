...
module.exports = CoreObject.extend({
  ...
  activate: function(revision) {
    ...
    .then(this._getSCMInfo.bind(this, revision))
    .then(function(scm){
  ...
  _getSCMInfo: function(targetRevision) {
    return new Promise(function(resolve, reject) {
      simpleGit().log([targetRevision], function(err, log) {
        if (err || !log || !log.latest) resolve();

        var regex = /(.*)\((.*), (.*)\)/;
        var message = log.latest.message.match(regex)[1].trim();
        var branch = log.latest.message.match(regex)[3];
        resolve({
